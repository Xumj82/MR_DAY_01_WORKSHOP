...
SELECT count(*) FROM ASD where Ethnicity = 'middle eastern' AND [Class/ASDTraits]='Yes';
/*  result: 96 */  

/*     How many children who have Jaundice show ASD traits ? */
SELECT count(*) FROM ASD where Jaundice = 'yes' AND [Class/ASDTraits]='Yes';
/*  reslut: 215 */

/*     Are ASD traits dependent on hereditary ? Justify . */
SELECT (SELECT (SELECT count(*) FROM ASD 
WHERE Family_mem_with_ASD='no' AND [Class/ASDTraits]='Yes')*1.0/(
SELECT count(*) from ASD WHERE Family_mem_with_ASD='no')) 
AS 'ASD proportion in family member without ASD',
(SELECT (SELECT count(*) FROM ASD 
WHERE Family_mem_with_ASD='yes' AND [Class/ASDTraits]='Yes')*1.0/(
SELECT count(*) from ASD WHERE Family_mem_with_ASD='yes'))
...
FROM ASD  WHERE [Class/ASDTraits]='Yes'
GROUP by Ethnicity
) as A
LEFT JOIN (
...
LEFT JOIN (
SELECT Ethnicity,count(*) As Amount
FROM ASD  WHERE  Ethnicity = 'White European' 
AND Sex = 'f'
GROUP by Ethnicity
) as B 
on A.Ethnicity = B.Ethnicity;
